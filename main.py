"""
MAGI System 2.0 - Main Application Server
Dash & Flask server managing retro Evangelion UI components, reactive consensus state callbacks,
and REST endpoints for audio transcription.
"""
...
app.layout = Div(
    ...
    children=[
        ...
        Div(className='input-container', children=[
            Label('access code: '),
            dcc.Input(id='key', autoComplete='off', type='password', value=os.getenv('OPENROUTER_API_KEY', os.getenv('OPENAI_API_KEY', ''))),
            Label('question: '),
            Div(className='query-wrapper', children=[
                dcc.Input(id='query', type='text', value='', debounce=True, autoComplete='off'),
                Button('音 声', id='stt-btn', className='evangelion-stt-btn', n_clicks=0),
                Button('発 送', id='send-btn', className='evangelion-send-btn', n_clicks=0),
                Button('デバッグ', id='debug-btn', className='evangelion-debug-btn', n_clicks=0),
            ]),
        ]),
        Div(id='stt-debug-log', className='stt-log-box stt-log-hidden', children='[STT STATUS: LISTO] Pulsa 音 声 para iniciar micrófono'),
...
@callback(
    ...
    State('key', 'value'),
    prevent_initial_call=True)
def wise_man_answer(question: dict, personality: str, wise_man_name: str, key: str):
    if question['error']:
        return {'id': question['id'], 'response': question['error'], 'status': 'error'}

    try:
        answer = ai.get_answer(question['query'], personality, key, wise_man_name=wise_man_name)

        if question['is_yes_or_no_question']:
            classification = ai.classify_answer(question['query'], personality, answer, key, wise_man_name=wise_man_name)
        else:
...
def api_transcribe():
    ...
    try:
        ...
        api_key = request.headers.get('X-API-Key') or os.getenv('STT_API_KEY') or os.getenv('OPENAI_API_KEY')
        if not audio_file:
